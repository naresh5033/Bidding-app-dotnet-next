...
{
    // just a helper class with initialize (all the auctions in the db table) and reinit (remove all the auctions in the db)
    ...
    {
        db.Auctions.RemoveRange(db.Auctions); //remove all the auctions from the table 
        db.SaveChanges();
        InitDbForTests(db);
    }
    ...
    private static List<Auction> GetAuctionsForTest()
    {
        ...
        {
            // 1 Ford GT //the auctions from our auction service seed class.
            new Auction
            {
