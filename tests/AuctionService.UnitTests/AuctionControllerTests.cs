...
public class AuctionControllerTests
{
    ...
    public AuctionControllerTests()
    {
        ...
        {
            mc.AddMaps(typeof(MappingProfiles).Assembly); //the mapping prof that we get from the auction service
        }).CreateMapper().ConfigurationProvider;

        _mapper = new Mapper(mockMapper);
        _controller = new AuctionsController(_auctionRepo.Object, _mapper, _publishEndpoint.Object) // controller with all its deps
        {
            ControllerContext = new ControllerContext
            {
                HttpContext = new DefaultHttpContext { User = Helpers.GetClaimsPrincipal() } //the claims from the utils
            }
        };
    }
    ...
    {
        // arrange
        var auctions = _fixture.CreateMany<AuctionDto>(10).ToList(); //these auctions will be created by the fixture with some random data
